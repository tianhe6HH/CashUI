...
DEFAULT_ADMIN = {"username": "admin", "password": DEFAULT_PASSWORD, "display_name": "管理员"}
...
def init():
    ...
    try:
        ...
        # 默认管理员（首次登录强制改密）
        if db.scalar(select(User).where(User.username == DEFAULT_ADMIN["username"])) is None:
            db.add(
                User(
            ...
            db.commit()
            print(f"已创建默认管理员：{DEFAULT_ADMIN['username']} / {DEFAULT_ADMIN['password']}（首次登录需改密）")
        else:
            print("默认管理员已存在，跳过")
